...
const ProductDetails = () => {
  ...
  const availableQuantity = productDetail?.quantity;

  const increaseCount = () => {
    if (count < availableQuantity) setCount(count + 1);
  };
  ...
  const { isLoading, mutate } = useMutation({
    ...
    mutationFn: async (value) => {
      return await $axios.post('/cart/add/item', value);
    },
    onSuccess: () => {
      router.push('/cart');
    },
        ...
        {isBuyer() && (
          <>
            ...
            <Button
              ...
              onClick={() => {
                const value = { productId: productId, orderQuantity: count };
                mutate(value);
              }}
            >
              Add to Cart
